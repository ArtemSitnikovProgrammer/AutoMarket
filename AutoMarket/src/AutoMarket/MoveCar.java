...
public class MoveCar extends BaseAI {

    boolean goingCar = true;
    boolean flag;
    String name;
    ...
    public static void pipedConnection(){
            ...
            try   {  pr = new PipedReader(MyFrame.MyConsolDialog.getStreamCar()); }
        catch(IOException e)        {
        ...
        while (goingCar)
        {
            ...
            synchronized (Habitat.Arr)
            {
                for (int i = 0; i < Habitat.Arr.size(); i++)
                {
                    ...
                    if(Habitat.Arr.get(i).getClass() == Car.class) {
                        if (Habitat.Arr.get(i).edy == true || Habitat.Arr.get(i).x <= Habitat.myFrame.panel.getWidth() / 2 || Habitat.Arr.get(i).y <= Habitat.myFrame.panel.getHeight() / 2) {
                            V = 5;
                            Habitat.Arr.get(i).edy = true;
                           // System.out.println("Движение Car");
